...
public:
    ...
    /**
     * @Function   遍历文件夹下的所有文件 以字母排序
     * @param path 文件夹路径
     * @param cb 回调对象 ，path为绝对路径，isDir为该路径是否为文件夹，返回true代表继续扫描，否则中断
     * @param enterSubdirectory 是否进入子目录扫描
     * @Programmer  HFauto
     */
    static void scanDirAlphasort(const std::string &path_in, const std::function<bool(const std::string &path, bool isDir)> &cb, bool enterSubdirectory = false);
