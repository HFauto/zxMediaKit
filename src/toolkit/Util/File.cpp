...
int filterDot(const struct dirent * dir) {
    if (File::is_special_dir(dir->d_name)) {
        return 0;
    } else {
        return 1;
    }
}
void File::scanDirAlphasort(const string &path_in, const function<bool(const string &path, bool isDir)> &cb, bool enterSubdirectory) {
    string path = path_in;
    if(path.back() == '/'){
        path.pop_back();
    }
    struct dirent **namelist;
    auto num = scandir(path.c_str(), &namelist, filterDot, alphasort);
    if(num < 0){
        return;
    }
    int idx = 0;
    while (idx != num) {
        if (namelist[idx]->d_type == DT_REG) {          //文件
            string filename = namelist[idx]->d_name;
            string strAbsolutePath = path + "/" + filename;
            if(!cb(strAbsolutePath, false)){
                //不再继续扫描
                break;
            }
        } else if (namelist[idx]->d_type == DT_DIR) {   //目录
            string filename = namelist[idx]->d_name;
            string strAbsolutePath = path + "/" + filename;
            if(!cb(strAbsolutePath, true)){
                //不再继续扫描
                break;
            }
            if(enterSubdirectory){                      //递归
                string strAbsolutePath = path + "/" + filename;
                scanDirAlphasort(strAbsolutePath,cb,enterSubdirectory);
            }
        }
        free(namelist[idx]);
        idx++;
