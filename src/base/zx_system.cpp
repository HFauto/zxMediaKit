...
void System::systemSetup(bool enCore){
#if !defined(_WIN32)
    ...
    if (getrlimit(RLIMIT_NOFILE, &rlim)==0) {
        ...
        DebugL << "文件最大描述符个数设置为:" << rlim_new.rlim_cur;
    }
    ...
    NoticeCenter::Instance().addListener(nullptr,kBroadcastOnCrashDump,[](BroadcastOnCrashDumpArgs){
        ...
        ofstream out(StrPrinter << exeDir() << "/crash/crash." << getpid(), ios::out | ios::binary | ios::trunc);
        out << stack_info;
        out.flush();
